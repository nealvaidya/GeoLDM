...
try:
    ...
    pass
import copy
from equivariant_diffusion.en_progdist import EnLatentDiffusion_Distiller
import utils
import argparse
import wandb
...
parser.add_argument("--anneal_steps", type=int, default=50_000)
...
# VAE args
parser.add_argument(
    "--latent_nf", type=int, default=4, help="number of latent features"
)
parser.add_argument(
    "--kl_weight", type=float, default=0.01, help="weight of KL term in ELBO"
)
...
parser.add_argument(
    "--diffusion_steps", type=int, default=500, help="student difusion steps"
)
parser.add_argument(
...
def compute_loss_and_nll(
    args, teacher_model, student_model, nodes_dist, x, h, node_mask, edge_mask, context
):
...
def main():
    ...
    teacher, nodes_dist, prop_dist = get_latent_diffusion(
        teacher_args, device, dataset_info, dataloaders["train"]
    )
    ...
    teacher_state_dict = torch.load(join(args.teacher_path, "generative_model_ema.npy"))
    ...
    student.T = args.diffusion_steps
    student.gamma = en_diffusion.PredefinedNoiseSchedule(
    ...
    model = EnLatentDiffusion_Distiller(student, teacher)
    ...
    if args.anneal_lr:
        scheduler = torch.optim.lr_scheduler.LinearLR(
            optim, 1, 0, total_iters=args.anneal_steps
        )
    else:
        scheduler = torch.optim.lr_scheduler.LinearLR(
            optim, 1, 1, 1
